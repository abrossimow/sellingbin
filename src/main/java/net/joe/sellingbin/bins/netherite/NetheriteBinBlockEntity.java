package net.joe.sellingbin.bins.netherite;
...
public class NetheriteBinBlockEntity extends BlockEntity implements NamedScreenHandlerFactory {
    ...
    public ScreenHandler createMenu(int syncId, PlayerInventory playerInventory, PlayerEntity player) {
        return new GenericContainerScreenHandler(ScreenHandlerType.GENERIC_9X6, syncId, playerInventory,
                SellingBinMod.inventoryManager.getPlayerInventory(player.getUuid()).getNetheriteBin(), 6);
    }
    ...
    public void sellItems(PlayerEntity player) {
        ...
        if (lastSellDay < day) {
            var playerInventory = SellingBinMod.inventoryManager.getPlayerInventory(player.getUuid()).getNetheriteBin();
    ...
    private void handleItemRemainders(PlayerEntity player, ItemStack itemStack, int slotCounter, int remainingAmount) {
        if (slotCounter > 26) {
            ...
            SellingBinMod.inventoryManager.getPlayerInventory(player.getUuid())
                    .getNetheriteBin().setStack(slotCounter, new ItemStack(itemStack.getItem(), remainingAmount));
        }
    }
    ...
    private void handleSoldItems(PlayerEntity player, String currencyName, int currencyAmount, ItemStack itemStack, int sellAmount, int slotCounter) {
        if (slotCounter > 26) {
            ...
            SellingBinMod.inventoryManager.getPlayerInventory(player.getUuid()).getNetheriteBin()
    ...
    private void handleUnmatchedItems(PlayerEntity player, ItemStack itemStack, int slotCounter) {
        if (slotCounter > 26) {
            ...
            SellingBinMod.inventoryManager.getPlayerInventory(player.getUuid()).getNetheriteBin().setStack(slotCounter, itemStack);
        }
    }
